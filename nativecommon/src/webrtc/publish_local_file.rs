...
pub async fn publish(
    ...
    let video_handle = tokio::task::spawn_blocking({
        ...
        let mut ticker = tokio::time::interval(Duration::from_nanos(1_000_000_000 / 60)); //FIXME: get from file
        ...
        move || {
            loop {
                ...
                let streaming_data = from_to_streaming_data_packets(
                    packets,
                    &metadata,
                    &stream_track,
                    &identity,
...
pub async fn publish3(
    ...
    let video_handle = tokio::spawn({
        ...
        log::debug!("listening video udp");
        let mut buf = [0u8; 2048];
        async move {
            loop {
                let (len, addr) = socket.recv_from(&mut buf).await?;
                log::debug!("received video udp packet from {addr}: {len} bytes");
                let packets = vec![buf[0..len].to_owned()];
                let streaming_data =
    ...
    let audio_handle = tokio::spawn({
        ...
        let socket = tokio::net::UdpSocket::bind("0.0.0.0:5002").await?;
        log::debug!("listening audio udp");
        let mut buf = [0u8; 2048];
        async move {
            loop {
                let (len, addr) = socket.recv_from(&mut buf).await?;
                log::debug!("received audio udp packet from {addr}: {len} bytes");
                let packets = vec![buf[0..len].to_owned()];
                let streaming_data =
                    from_to_streaming_data(packets, &metadata, &stream_track, &identity, &record);
                streaming_data_sender
...
fn from_to_streaming_data_packets(
